...
def cost_function(x, target, buttons, joltage):
    s0 = [0]*len(joltage)
    c = (10*diff_cost(state(s0, buttons, x), joltage))**2 + abs(sum([round(b) for b in x]))**2
#    c = (1000*diff_cost(state(s0, buttons, x), joltage))**2
    return c
...
for target, buttons, joltage in zip(targets, buttons_set, joltages):
    bounds = [(0,max(joltage))]*len(buttons)
    result = differential_evolution(func=cost_function, bounds=bounds, args=(target, buttons, joltage), popsize=10000, integrality=[True]*len(buttons))
